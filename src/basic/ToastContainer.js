...
class ToastContainer extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      modalVisible: false,
      _panResponder: this._createPanResponder()
    };
  ...
  showToast({ config }) {
    this.setState({
      ...
      _panResponder: this._createPanResponder(config.panOffset),
      ...(config.panOffset && {
        pan: new Animated.ValueXY({ x: 0, y: 0, ...config.panOffset })
      })
    });
  ...
  _createPanResponder(panOffset) {
    return PanResponder.create({
      onMoveShouldSetPanResponderCapture: () => true,
      onPanResponderRelease: (evt, { dx }) => {
        if (dx !== 0) {
          Animated.timing(this.state.pan, {
            toValue: { x: dx, y: 0, ...panOffset },
            duration: 100,
            useNativeDriver: false
